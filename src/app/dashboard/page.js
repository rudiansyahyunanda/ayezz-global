...
export default function UserDashboardPage() {
  ...
  // Sidebar active tab state: 'overview' | 'new-order' | 'orders' | 'invoices' | 'profile'
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans antialiased flex flex-col select-none">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          ...
          <div className="p-5 border-b border-slate-800 bg-slate-950/50 flex items-center space-x-3.5">
            <div className="w-11 h-11 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-sm font-black font-mono text-amber-400 shrink-0 shadow-inner">
              {avatarInitials}
            </div>
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] font-mono text-slate-400 truncate pt-0.5">
                {user?.email}
              </p>
            </div>
          </div>
            ...
            >
              ...
              {orders.length > 0 && (
                <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full font-bold ${
                  activeTab === 'orders' ? 'bg-slate-950 text-white' : 'bg-slate-800 text-slate-300'
                }`}>
                  {orders.length}
                </span>
              )}
          ...
          <div className="p-4 border-t border-slate-800 space-y-2">
            <Link
              href="/katalog"
              className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold uppercase tracking-wider rounded-xl transition-all flex items-center justify-center space-x-2"
            >
            ...
            <button
              ...
              className="w-full py-2.5 bg-rose-950/40 hover:bg-rose-900/60 text-rose-300 text-xs font-bold uppercase tracking-wider rounded-xl transition-all flex items-center justify-center space-x-2 border border-rose-900/40"
            >
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
          ...
          <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-6 py-4 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <button
              ...
              <div>
                <h2 className="text-base font-extrabold text-slate-900 uppercase tracking-tight">
                  ...
                  {activeTab === 'new-order' && 'Konfigurasi & Tempahan Pesanan'}
                  {activeTab === 'orders' && 'Sejarah Pesanan Pengguna'}
                  {activeTab === 'invoices' && 'Invois & Resit Rasmi'}
                  {activeTab === 'profile' && 'Tetapan Profil & Alamat Akaun'}
                </h2>
                <p className="text-[11px] text-slate-500 font-medium hidden sm:block">
                  Panel Kawalan Pelanggan Kilang Sublimasi AYEZZ GLOBAL
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              <button
                ...
                className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all flex items-center space-x-1.5 shadow-xs active:scale-95"
              >
          ...
          <main className="p-6 sm:p-10 flex-1 space-y-8 max-w-7xl w-full mx-auto pb-24 md:pb-10">
            ...
            {activeTab === 'overview' && (
              <div className="space-y-8">
                {/* WELCOME BANNER */}
                <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white p-8 rounded-3xl border border-slate-800 shadow-lg relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                  ...
                  <button
                    onClick={() => setActiveTab('new-order')}
                    className="px-6 py-3.5 bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-xs uppercase tracking-widest rounded-2xl transition-all shadow-md active:scale-95 flex items-center space-x-2 shrink-0 relative z-10"
                  >
                            ...
                            <tr key={ord.id} className="hover:bg-slate-50/80 transition-colors">
                              ...
                              <td className="py-3.5 px-3 text-right">
                                <button
                                  ...
                                  className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-900 rounded-lg text-[11px] font-bold transition-colors"
                                >
                                  Butiran
                                </button>
                              </td>
                  ...
                  <div className="bg-white p-8 rounded-3xl border border-slate-200/80 shadow-md text-center space-y-6">
                    ...
                    <div className="flex items-center justify-center space-x-3 pt-2">
                      <button
                        ...
                        className="px-6 py-3.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs uppercase tracking-widest rounded-2xl transition-all shadow-md"
                      >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                        {Object.keys(sizeQuantities).map((sz) => (
                          <div key={sz} className="p-3 bg-slate-50 border border-slate-200 rounded-2xl text-center space-y-2">
                            <span className="text-xs font-mono font-bold text-slate-700 block">{sz}</span>
                            <div className="flex items-center justify-center space-x-2">
                              <button
                                type="button"
                                onClick={() => updateSizeQty(sz, -1)}
                                className="w-6 h-6 rounded-full bg-white border border-slate-300 text-xs font-bold text-slate-800 hover:bg-slate-200 active:scale-95"
                              >
                                -
                              </button>
                              ...
                              <button
                                type="button"
                                onClick={() => updateSizeQty(sz, 1)}
                                className="w-6 h-6 rounded-full bg-slate-900 text-white text-xs font-bold hover:bg-slate-800 active:scale-95"
                              >
                                +
                              </button>
                            </div>
                  ...
                  {filteredOrders.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {filteredOrders.map((ord) => (
                        <div
                          key={ord.id}
                          className="p-5 bg-white border border-slate-200 rounded-2xl hover:border-slate-400 hover:shadow-md transition-all flex flex-col md:flex-row items-start md:items-center justify-between gap-5"
                        >
                          ...
                          <div className="flex items-center space-x-4 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-3 md:pt-0 border-slate-100">
                            ...
                            <button
                              onClick={() => setSelectedOrder(ord)}
                              className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-colors flex items-center space-x-1"
                            >
                            ...
                            <tr key={ord.id} className="hover:bg-slate-50/80 transition-colors">
                              ...
                              <td className="py-3.5 px-3 text-right">
                                <button
                                  ...
                                  className="px-3.5 py-1.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-[11px] font-bold transition-colors inline-flex items-center space-x-1"
                                >
              ...
              <div className="max-w-2xl mx-auto bg-white border border-slate-200/80 rounded-3xl p-8 shadow-xs space-y-6">
                ...
                <form onSubmit={handleSaveProfile} className="space-y-5">
                  ...
                  <button
                    ...
                    className="w-full py-3.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs uppercase tracking-widest rounded-xl transition-all flex items-center justify-center space-x-2"
                  >
                    {isSavingProfile ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 space-y-6 shadow-2xl relative border border-slate-200 font-sans">
            <button
              ...
              className="absolute top-5 right-5 p-2 bg-slate-100 hover:bg-slate-200 rounded-full transition-colors"
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-10 space-y-6 shadow-2xl relative border border-slate-200 font-sans max-h-[92vh] overflow-y-auto">
            <button
              ...
              className="absolute top-5 right-5 p-2 bg-slate-100 hover:bg-slate-200 rounded-full transition-colors print:hidden"
            >
              <X className="w-4 h-4 text-slate-600" />
            </button>
            ...
            <div className="grid grid-cols-2 gap-6 bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs font-mono">
              <div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-200">
              ...
              <div className="flex items-center space-x-3 print:hidden">
                <button
                  ...
                  className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold uppercase tracking-wider inline-flex items-center space-x-1.5 shadow-xs"
                >
