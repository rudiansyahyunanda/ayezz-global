...
export default function AdminDashboard({ onSwitchToStorefront }) {
  ...
  const fetchAllData = async () => {
    setIsLoading(true);
    try {
      const [cats, cuts, fabs, tpls, ords, settings] = await Promise.all([
        getCategories(),
        getCutTypes(),
        getFabricTypes(),
        getDesignTemplates(),
      ...
      <aside className="w-64 bg-[#0F172A] text-white flex flex-col justify-between shrink-0 z-30 shadow-xl border-r border-slate-800">
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto px-3 py-6 space-y-6 text-xs font-medium">
            ...
            <div className="space-y-1">
              <span className="px-3 text-[11px] font-mono font-medium text-slate-400 uppercase tracking-wider block mb-2">JUALAN</span>
